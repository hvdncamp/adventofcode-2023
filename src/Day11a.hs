...
distance (Coordinate x1 y1) (Coordinate x2 y2) (xs,ys) = sum [if xi `elem` xs then 2 else 1 | xi<-[(x1 + 1)..x2]] 
                                                       + sum [if yi `elem` ys then 2 else 1 | yi<-[(y1 + 1)..y2]]
...
antwoord = do
    ...
    let galaxyList = findGalaxies $ readGrid $ lines input in 
        print $ totalDistance galaxyList
